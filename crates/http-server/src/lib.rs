...
pub async fn handle_component_request(
    ...
    let plugin = store
        .data()
        .active_ctx
        .get_plugin::<AuthzEngine>(MESHX_AUTHZ_ID);

    // Get the stable request ID for context tracking
    let request_id = store.data().active_ctx.id.clone();
    ...
    // Clone plugin for cleanup in spawned task
    let plugin_for_cleanup = plugin.clone();
    ...
    let task: JoinHandle<anyhow::Result<()>> = tokio::task::spawn(async move {
        ...
        // Clean up the request principal after the request completes
        if let Some(plugin) = plugin_for_cleanup {
            plugin
                .clear_request_principal(&request_id_for_cleanup)
                .await;
        }
